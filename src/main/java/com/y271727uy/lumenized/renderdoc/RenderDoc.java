package com.y271727uy.lumenized.renderdoc;
	...
	static {
		...
		if (osName.contains("win") || !osName.contains("mac")) {
			try {
				...
				if (initResult != 1) {
					LumenizedConstants.LOGGER.error("Could not connect to RenderDoc API, return code: {}", initResult);
				} else {
					...
					LumenizedConstants.LOGGER.info("Connected to RenderDoc API v" + major.getValue() + "." + minor.getValue() + "." + patch.getValue());
				}
			} catch (UnsatisfiedLinkError ignored) {
			}
		}
		LumenizedConstants.LOGGER.info("renderDoc available:" + (apiInstance == null ? "failed" : "success"));
