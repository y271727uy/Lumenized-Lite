...
public class LumenizedForgeConfig {
    ...
    private static void logAccessUnInit(String configValueName){
        LumenizedConstants.LOGGER.error("trying to access uninitialized lumenized config value:{}," +
            "see stacktrace at debug log file",configValueName);
        LumenizedConstants.LOGGER.debug("{} thread stacktrace",Thread.currentThread().getName());
        for (StackTraceElement element : Thread.currentThread().getStackTrace()) {
            LumenizedConstants.LOGGER.debug(element);
        }
    }
    ...
    private static void registerClientConfig(ForgeConfigSpec.Builder builder){
        ...
        """
                whether enable buildin lumenized.json
                """
